...
int64_t BUStep(const Graph &g, pvector<NodeID> &parent, Bitmap &front, Bitmap &next) {
...
int64_t TDStep(const Graph &g, pvector<NodeID> &parent, SlidingQueue<NodeID> &queue) {
...
pvector<NodeID> DOBFS(const Graph &g, NodeID source, bool logging_enabled = false, int alpha = 15, int beta = 18) {
    if (logging_enabled)
...
int main(int argc, char *argv[]) {
    ...
    auto BFSBound = [&sp, &cli](const Graph &g) { return DOBFS(g, sp.PickNext(), cli.logging_en()); };
